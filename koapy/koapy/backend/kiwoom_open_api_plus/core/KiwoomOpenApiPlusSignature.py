...
class KiwoomOpenApiPlusSignature(Signature):
    ...
    COMTYPE_TO_PYTHONTYPE = {
        ...
        pythoncom.VT_VARIANT: type(None), # QVariant does not exist in PySide2 (Currently PyQt5 only)
        pythoncom.VT_VOID: type(None),
    }

    def __init__(self, name, parameters=None, return_annotation=Signature.empty, entry=None):
        self._name = name
        self._entry = entry
